...
public class Controller7 {//класс контроллер формы изменение данных клиента
    ...
    public void setPerson(Client client) {//получение данных клиента
        this.client = client;

        lastNameField.setText(client.getFirstName());
        firstNameField.setText(client.getLastName());
        patronymicField.setText(client.getPatronymic());
        datepick.setValue(client.getBirthDate().toLocalDate());
        pasportField.setText(client.getPasport());
        numberPhoneField.setText(client.getNumberPhone());
    ...
    public void handleOk(javafx.event.ActionEvent event) {//нажатие кнопки Ок
        ...
        if (isInputValid()) {
            ...
            try {
                ...
                mainController.DialogInfo("Данные клиента успешно изменены!");


            } catch (Exception e) {
                mainController.DialogError("Не удалось изменить данные клиента!");
